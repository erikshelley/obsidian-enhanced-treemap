...
var d3Template = `
    ...
    var treemapLayout = d3.treemap().size([400, 500]).paddingOuter(padding).paddingTop(fontsize + 1.5 * padding).paddingInner(padding)(nodes);
    ...
    svg.selectAll("label").data(nodes.descendants().filter(function(d) { return d.children; })).enter()
        .append("text")
            .attr("x", function(d) { return d.x0 + 0.5 * (d.x1 - d.x0); })
            .attr("y", function(d) { return d.y0 + fontsize + 0.5 * padding; })
            .attr("text-anchor", "middle")
            .text(function(d) { return d.data.name; })
            .attr("font-size", fontsize + "px")
